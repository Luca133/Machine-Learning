...
temp = np.array([[1200.77777, 4400.44444, 5000.55555],
                [4400.44444, 47040.33333, 51000.88888],
                [5000.555555, 51000.88888, 56000.99999]])

tempDet = np.linalg.det(temp)
...
for t in range(n_iter):
    ...
    for i in range(k):
        ...
        # Method 1 - Create Identity matrix, multiply by small value and add it to the covariance matrices
        # Thus ensuring that the diagonal values are never 0
        epsilon = np.identity(D) * 0.001
        s[i] += epsilon

        # Method 2 - Check to see if covariance matrix is invertible
        # If it is not then reset mean (mu) and covariance matrix to arbitrary values
        # a = s[i]
        # determinant = np.abs(np.linalg.det(a))
        # if determinant <= 0.00001 or np.isnan(determinant):
        #     random_indices = np.floor(N * np.random.rand((k)))
        #     random_indices = random_indices.astype(int)
        #     cov_matrix = np.cov(X.transpose())
        #     s[i, :, :] = cov_matrix / k
